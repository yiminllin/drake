...
multibody::SpatialVelocity<double> rolling_pin_velocity(double t) {
    multibody::SpatialVelocity<double> cylinder_velocity;
    if (t < 1.5) {
        cylinder_velocity.translational() = Vector3<double>(-0.25, 0.0, 0.0);
        // Choose consistent rotational velocity so
        // rotational velocity x (0, 0, r) = translational velocity
        cylinder_velocity.rotational() = Vector3<double>(0.0, -10.0, 0.0);
    } else if (t < 1.7) {
        cylinder_velocity.translational() = Vector3<double>(0.0, 0.0, -0.1);
        cylinder_velocity.rotational() = Vector3<double>(0.0, 0.0, 0.0);
    } else if (t < 3.2) {
        cylinder_velocity.translational() = Vector3<double>(0.25, 0.0, 0.0);
        cylinder_velocity.rotational() = Vector3<double>(0.0, 10.0, 0.0);
    } else if (t < 3.3) {
        ...
        cylinder_velocity.translational() = Vector3<double>(0.0, 0.0, 0.0);
        cylinder_velocity.rotational() = Vector3<double>(0.0, 0.0, 5.0*M_PI/4.0);
    } else {
        cylinder_velocity.translational() = Vector3<double>(-0.2/sqrt(2), -0.2/sqrt(2), 0.0);
        cylinder_velocity.rotational() = Vector3<double>(-8.0/sqrt(2), 8.0/sqrt(2), 0.0);
    }
    return cylinder_velocity;
}
...
int DoMain() {
    ...
    // // rotational velocity x (0, 0, -r) = - translational velocity
    // cylinder_velocity.rotational() = Vector3<double>(0.0, -10.0, 0.0);
    Vector3<double> cylinder_translation = {0.6, 0.1, 0.08};
    double cylinder_radius = 0.025;
    double cylinder_mu = 0.5;
    double wall_mu = 0.8;
    double E = 9e4;
    ...
    MPMParameters::IOParameters io_param {
        "mpm-test",                            // case name
        "/home/yiminlin/Desktop/drake/multibody/fem/mpm-dev/examples/dough-rolling/outputs-plastic-lowres",       // output directory name
    ...
    std::unique_ptr<SpatialVelocityTimeDependent> cylinder_velocity_ptr =
        std::make_unique<SpatialVelocityTimeDependent>(rolling_pin_velocity);
